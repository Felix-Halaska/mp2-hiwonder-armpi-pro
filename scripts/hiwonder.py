...
from utils import EndEffector, rotm_to_euler, dh_to_matrix
import utils as ut
import sympy as sp
...
class HiwonderRobot:
    ...
    def set_arm_velocity(self, cmd: ut.GamepadCmds):
        ...
        invJac = np.array(
            sp.transpose(self.jacobian)
            * (
                ((self.jacobian * sp.transpose(self.jacobian)) + sp.eye(3) * 0.0001)
                ** -1
            )
        )
        ...
        for i in range(5):
            new_thetalist[i] = self.joint_values[i] + dt * 0.05 * np.rad2deg(
                float((thetaDot[i][0]))
            )  # thetalist_dot[i]
    ...
    def calc_HTM(self):
        ...
        arr = []
        dhTable = [
            [self.joint_values[0], self.l1, 0, math.pi / 2],
            [math.pi / 2, 0, 0, 0],
            [self.joint_values[1], 0, self.l2, math.pi],
            [self.joint_values[2], 0, self.l3, math.pi],
            [self.joint_values[3], 0, self.l4, 0],
            [-math.pi / 2, 0, 0, -math.pi / 2],
            [self.joint_values[4], self.l5, 0, 0],
        ]
